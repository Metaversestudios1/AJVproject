...
const login = async (req, res) => {
    ...
    try {
        ...
        // Check if the provided role matches the admin's role
        if (admin.role !== role) { // Assuming `role` is a field in your admin model
            return res.status(403).json({ success: false, message: "Role does not match" });
        }
        ...
        const token = jwt.sign(
            { id: admin._id, username: admin.username, role: admin.role }, // Include role in the token payload if needed
...
const updateAdmin = async(req,res)=>{
    ...
    try{
        ...
        res.status(500).json({success:false,message:"error in updating the Admin",error:err.message});
